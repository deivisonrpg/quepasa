    ...
    <div class="page-header">
      <div class="page-title-row">
        ...
        <div v-if="!loading && hasServers" class="metric-chips">
          <button type="button" class="mchip mchip-total" :class="{ active: statusFilter === 'total' }" @click="statusFilter = 'total'" :aria-pressed="statusFilter === 'total'">{{ servers.length }} {{ t('total') }}</button>
          <button type="button" class="mchip mchip-online" :class="{ active: statusFilter === 'connected' }" @click="statusFilter = 'connected'" :aria-pressed="statusFilter === 'connected'">{{ connectedCount }} {{ t('connected') }}</button>
          <button type="button" class="mchip mchip-offline" :class="{ active: statusFilter === 'disconnected' }" @click="statusFilter = 'disconnected'" :aria-pressed="statusFilter === 'disconnected'">{{ disconnectedCount }} {{ t('disconnected') }}</button>
        </div>
      </div>
...
export default defineComponent({
  setup() {
    ...
    // Reset to page 1 when search, page size, or status filter changes
